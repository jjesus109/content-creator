...
-- Note: uses external_post_id (platform-native post ID) — no Ayrshare dependency.
...
-- confirm that a post is visible 30 minutes after publishing.
--
-- status values:
--   'published'     — Platform API accepted the post
--   'failed'        — Platform API rejected the post or was unreachable
...
--
-- external_post_id: Platform-native post ID returned by direct platform API
--   (YouTube video ID, Instagram media ID, Facebook video ID).
--   Used by the verify job to confirm live status.
-- platform_post_id: Public URL or identifier for the post (e.g. YouTube Shorts URL).
